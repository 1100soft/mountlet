...
fn notice_state_path() -> Option<PathBuf> {
    let directory = mountlet_state_dir()?;
    let mut candidates = fs::read_dir(&directory)
        ...
        .filter(|path| {
            path.file_name()
                ...
                .is_some_and(|name| name.starts_with("notices-") && name.ends_with(".json"))
        })
        .collect::<Vec<_>>();
    candidates.sort_by_key(|path| fs::metadata(path).and_then(|meta| meta.modified()).ok());
    candidates
        .pop()
        .or_else(|| Some(directory.join("notices-production.json")))
}
...
fn fetch_and_remember_notices() -> Result<Vec<NoticeView>, String> {
        ...
        .flatten()
    {
        ...
        history.insert(key, remembered);
    }
    save_notice_state(&path, &state)?;
    notification_history().map(|values| values.into_iter().filter(|notice| !notice.seen).collect())
}
...
fn persist_browser_memory_value(memory: serde_json::Value) -> Result<serde_json::Value, String> {
    ...
    if let Some(object) = current.as_object_mut() {
        if let Some(paths) = memory.get("paths") {
            object.insert("paths".into(), paths.clone());
        }
        if let Some(selections) = memory.get("selections") {
            object.insert("selections".into(), selections.clone());
        }
    }
...
fn app_settings() -> AppSettings {
    ...
    AppSettings {
        ...
        window_mode: platform::effective_window_mode(&choice("ui", "window_mode", "multiple", &["single", "multiple"])),
...
fn shortcut_preferences() -> HashMap<String, Vec<String>> {
    ...
    let mut values: HashMap<String, Vec<String>> = defaults
        .into_iter()
        .map(|(key, value)| {
            (
                key.into(),
    ...
    for raw in text.lines() {
        ...
        if !values.contains_key(key.trim()) {
            continue;
        }
...
fn create_bug_report(
    ...
    let mut report = app_diagnostics(window, state.clone());
    report.push_str("\nRecent rclone output:\n");
    report.push_str(&rclone_output(state.clone()));
    for (label, source) in [
        ("Mountlet settings", app_config_path()),
        (
            "Mount settings",
            mountlet_config_dir().map(|path| path.join("mounts.toml")),
        ),
        ("rclone config", state.rclone_config.clone()),
    ] {
...
fn unreported_crash() -> Result<String, String> {
    ...
    let Some(marker) = text.rfind("Unhandled Rust panic") else {
...
async fn submit_bug_report(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let payload = serde_json::json!({
            ...
            "message": redact_sensitive_text(&message).chars().take(8000).collect::<String>(),
            "contact": contact.trim().chars().take(240).collect::<String>(),
            "metadata": {"appVersion":env!("CARGO_PKG_VERSION"),"buildChannel":"preview","buildId":env!("CARGO_PKG_VERSION"),"platform":format!("{} {}",env::consts::OS,env::consts::ARCH),"rust":true,"node":env::var("HOSTNAME").unwrap_or_default(),"frozen":true},
            "logs": if include_logs { serde_json::json!({"runtime":redact_sensitive_text(if crash.is_empty() { &diagnostics } else { &crash }),"rclone":redact_sensitive_text(&rclone)}) } else { serde_json::json!({}) }
        });
        let endpoint = env::var("MOUNTLET_REPORT_API_URL").unwrap_or_else(|_| "https://mountlet.app/api/report".into());
        let response = reqwest::blocking::Client::builder().timeout(Duration::from_secs(15)).build().map_err(|error| error.to_string())?
            .post(endpoint).header("User-Agent", format!("Mountlet/{} (+https://mountlet.app)", env!("CARGO_PKG_VERSION"))).json(&payload).send()
            .map_err(|error| format!("Could not reach report server: {error}"))?;
        let status = response.status();
        let value: serde_json::Value = response.json().map_err(|_| "Report server returned an invalid response.")?;
        if !status.is_success() || !value.get("ok").and_then(|item| item.as_bool()).unwrap_or(false) {
            return Err(value.get("error").and_then(|item| item.as_str()).unwrap_or("Report was not accepted.").to_string());
        }
        if kind == "crash" { mark_crash_reported(crash)?; }
        Ok(value.get("issueUrl").or_else(|| value.get("url")).and_then(|item| item.as_str()).unwrap_or("").to_string())
        ...
        .filter_map(|(key, raw)| {
            ...
            Some(NoticeView {
                ...
                seen: raw
                    ...
                    .and_then(|value| value.as_bool())
                    .unwrap_or(false)
                    || seen.is_some_and(|values| values.contains_key(key)),
                deletable: level != "critical" && notice_type != "price",
                received_at: raw
                    .get("receivedAt")
                    .and_then(|value| value.as_i64())
                    .unwrap_or(0),
...
fn create_remote(
    ...
    if matches!(request.provider.as_str(), "drive" | "gphotos" | "dropbox" | "onedrive" | "box" | "pcloud")
...
#[tauri::command]
fn push_config_sync(password: String, state: tauri::State<'_, AppState>) -> Result<(), String> {
...
pub(crate) fn cache_tray_anchor(app: &tauri::AppHandle, x: f64, y: f64) {
    let tray_box = app.tray_by_id("mountlet").and_then(|tray| tray.rect().ok().flatten());
    ...
    if let Ok(mut anchor) = state.window_anchor.lock() {
        ...
        if suspicious && tray_box.is_none() {
            return;
        }
...
fn apply_window_layout(
    ...
    let platform_area = work_area::resolve(monitor_area.unwrap_or(requested_area));
    ...
    let anchored = state.window_anchor.lock().ok().and_then(|mut anchor| {
        ...
        if anchor.edge.is_empty() || anchor.area_signature != signature {
            let right = platform_area.x + platform_area.width;
            let bottom = platform_area.y + platform_area.height;
            let distances = [
                ("left", (point.0 - platform_area.x).abs()),
                ("right", (point.0 - right).abs()),
                ("top", (point.1 - platform_area.y).abs()),
...
fn gphotos_mutation_error(remote_id: &str, path: &str, action: &str) -> Result<(), String> {
    ...
    if action == "mkdir" {
        let parent = path
            ...
            .map(|(value, _)| value)
            .unwrap_or("");
        if parent.eq_ignore_ascii_case("album") || path.trim_matches('/').eq_ignore_ascii_case("album") {
            return Ok(());
        }
...
async fn transfer_entry(
    ...
    let destination_provider = rclone_section_value(&request.destination_remote_id, "type").unwrap_or_default();
    if destination_provider == "gphotos"
        && !platform::gphotos_album_writable(&request.destination_path)
    {
        return Err("Google Photos can receive media only in albums created through rclone.".into());
    }
...
async fn remember_selection(
    ...
    if let Some(object) = memory.as_object_mut() {
        if let Some(paths) = object
            ...
            .as_object_mut()
        {
            paths.insert(request.remote_id.clone(), serde_json::json!(request.path.clone()));
        }
...
pub(crate) fn show_window_stack(app: &tauri::AppHandle) {
    ...
    // The main remote list owns keyboard navigation after a tray activation.
...
fn build_tray_menu<M: Manager<tauri::Wry>>(app: &M) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let export_config = MenuItem::with_id(app, "export-config", "Export config bundle", true, None::<&str>)?;
    let import_config = MenuItem::with_id(app, "import-config", "Import config bundle", true, None::<&str>)?;
