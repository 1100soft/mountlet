import "./style.css";
import { createBugReport, exportConfigBundle, importConfigBundle, markCrashReported, pullConfigSync, pushConfigSync, submitBugReport, unreportedCrash } from "./backend.ts";
...
interface ContextAction { label?: string; enabled?: boolean; run?: () => void; separator?: boolean }
...
function showContextMenu(event: MouseEvent, actions: ContextAction[]): void {
  ...
  for (const action of actions) {
    if (action.separator) { menu.append(element("hr")); continue; }
  ...
  const dismiss = (dismissEvent: Event) => {
    if (!(dismissEvent.target as Element | null)?.closest?.(".context-menu")) menu.remove();
  };
...
function persistBrowserFolder(remoteId: string, folderPath: string, itemPath = "", index = 0): void {
  browserMemory = {
    ...browserMemory,
  ...
  void persistBrowserMemory(browserMemory);
}

function persistBrowserPath(remoteId: string, folderPath: string): void {
  browserMemory = {
    ...browserMemory,
    paths: { ...browserMemory.paths, [remoteId]: folderPath },
  };
...
function matchesShortcut(event: KeyboardEvent, action: string): boolean {
  ...
  const key = ({ ArrowUp: "up", ArrowDown: "down", ArrowLeft: "left", ArrowRight: "right", " ": "space", Enter: "enter", Escape: "escape" } as Record<string, string>)[event.key]
    ?? (event.key === "=" && event.ctrlKey ? "+" : event.key.toLocaleLowerCase());
  if (!["control", "alt", "shift", "meta"].includes(key)) parts.push(key);
  const actual = parts.join("+");
  return (shortcuts[action] ?? []).some(value => value.toLocaleLowerCase().replaceAll("return", "enter").replaceAll("arrow", "") === actual);
}
...
async function showNotifications(): Promise<void> {
  ...
  const renderNotifications = async () => {
    ...
    if (!notices.length) { list.replaceChildren(element("p", "notification-empty", "No new notifications")); return; }
    list.replaceChildren(...notices.map(notice => {
      const card = element("article", `notification-card level-${notice.level}${notice.seen ? " seen" : " unseen"}`);
      const heading = element("button", "notification-title", notice.title); heading.type = "button";
      const body = element("p", "notification-message", notice.message);
      heading.addEventListener("click", async () => {
        await markNotificationSeen(notice.key); card.classList.replace("unseen", "seen"); void refreshNotificationIndicator();
        if (notice.url) await openExternal(notice.url);
      });
      card.append(heading, body);
      if (notice.deletable) {
        const remove = element("button", "notification-delete", "Delete"); remove.type = "button";
        remove.addEventListener("click", async () => { if (await deleteNotification(notice.key)) { await renderNotifications(); await refreshNotificationIndicator(); } });
        card.append(remove);
      }
      return card;
    }));
  ...
  dialog.append(title, list, element("div", "dialog-actions"));
  dialog.querySelector(".dialog-actions")!.append(close);
...
async function pollNoticeServer(): Promise<void> {
  ...
  try {
    ...
    for (const notice of notices) {
      ...
      if (notice.level === "critical" || display === "dialog") openDialog = true;
    ...
    if (openDialog) await showNotifications();
...
async function showLicense(): Promise<void> {
  ...
  const renderLicense = async () => {
    const status = await licenseStatus(); currentLicense = status; const summary = element("p", "license-summary", status.summary); content.replaceChildren(summary);
    if (status.state !== "licensed") {
      const form = element("div", "license-form"); const key = element("input", "") as HTMLInputElement; key.placeholder = "License key";
      const label = element("input", "") as HTMLInputElement; label.placeholder = "Device label (optional)";
      const activate = element("button", "primary", "Activate"); activate.addEventListener("click", async () => { activate.disabled = true; try { currentLicense = await activateLicense(key.value, label.value); render(); if (preferences.mode === "multiple") { await setDetachedBrowser(true); await emitBrowserState(selectedRemote, currentPath); } scheduleNativeLayout(0); await renderLicense(); } catch (error) { await showError("License activation", error); } finally { activate.disabled = false; } });
      const buy = element("button", "", "Buy license"); buy.addEventListener("click", () => void openExternal("https://mountlet.app/#pricing"));
      form.append(key, label, activate, buy); content.append(form); return;
    }
    const details = element("dl", "remote-summary");
    for (const [name, value] of [["Email", status.licensedEmail], ["Plan", status.plan], ["Device", status.deviceLabel], ["Expires", status.expiresAt]]) if (value) details.append(element("dt", "", name), element("dd", "", value));
    content.append(details); const devicesButton = element("button", "", "Manage devices");
    const addDevices = element("button", "", "Add devices"); addDevices.addEventListener("click", () => void openExternal(`https://mountlet.app/?license_action=add_devices&license_key=${encodeURIComponent(status.licenseKey)}#pricing`));
    devicesButton.addEventListener("click", async () => { devicesButton.disabled = true; try {
      const result = await licenseDevices(); const devices = Array.isArray(result.devices) ? result.devices as Array<Record<string, unknown>> : [];
      const list = element("div", "license-devices");
      for (const device of devices) { const row = element("div", "license-device"); row.append(element("span", "", String(device.deviceLabel ?? device.label ?? device.deviceId ?? "Device"))); const remove = element("button", "", "Deactivate"); remove.addEventListener("click", async () => { const id = String(device.deviceId ?? device.id ?? ""); if (!await confirmOwned("Deactivate device?", `Deactivate ${String(device.deviceLabel ?? "this device")}?`, "Deactivate")) return; await deactivateLicenseDevice(id); await renderLicense(); }); row.append(remove); list.append(row); }
      content.append(list);
    } catch (error) { await showError("License devices", error); } finally { devicesButton.disabled = false; } });
    const deactivate = element("button", "", "Deactivate this device"); deactivate.addEventListener("click", async () => { if (!await confirmOwned("Deactivate license?", "Deactivate Mountlet on this device?", "Deactivate")) return; await deactivateLicenseDevice(); currentLicense = await licenseStatus(); await setDetachedBrowser(false); render(); scheduleNativeLayout(0); await renderLicense(); });
    content.append(devicesButton, addDevices, deactivate);
  };
  await renderLicense();
  trapModalFocus(layer, dialog, close);
...
function showApplicationMenu(label: string, event: MouseEvent): void {
  ...
  if (label === "App") showContextMenu(event, [
    ...
    { label: "Set config sync location", run: () => void showAppSettings() }, { label: "Push config to sync location", run: () => void syncConfiguration("push") },
...
async function showShortcuts(): Promise<void> {
  ...
  openShortcutDialog(settings, async next => {
    ...
    shortcuts = completeSettings.shortcuts;
  });
}
...
async function showAbout(): Promise<void> {
  ...
  const output = element("pre", "rclone-output", await appDiagnostics()); output.tabIndex = 0;
  const actions = element("div", "dialog-actions");
  const website = element("button", "", "Website"); website.addEventListener("click", () => void openExternal("https://mountlet.app"));
  const copy = element("button", "", "Copy diagnostics"); copy.addEventListener("click", () => void navigator.clipboard.writeText(output.textContent ?? ""));
  const close = element("button", "primary", "Close"); close.addEventListener("click", () => layer.remove());
  actions.append(website, copy, close); dialog.append(output, actions); layer.append(dialog); document.body.append(layer);
  layer.addEventListener("keydown", event => { if (event.key === "Escape") layer.remove(); });
  trapModalFocus(layer, dialog, close);
}
...
async function reportBug(crash = ""): Promise<void> {
  ...
  const contact = element("input", "") as HTMLInputElement; contact.placeholder = "Optional contact email";
  const message = element("textarea", "report-message") as HTMLTextAreaElement; message.placeholder = "Describe what happened";
  ...
  const preview = element("pre", "rclone-output", [await appDiagnostics(), crash].filter(Boolean).join("\n\n"));
  const status = element("p", "dialog-status"); const actions = element("div", "dialog-actions");
  const save = element("button", "", "Save report locally"); save.addEventListener("click", async () => { const path = await createBugReport(); await navigator.clipboard.writeText(path).catch(() => undefined); status.textContent = `Saved to ${path}`; });
  ...
  actions.append(save, send, close); dialog.append(contact, message, logsLabel, preview, status, actions); layer.append(dialog); document.body.append(layer);
  layer.addEventListener("keydown", event => { if (event.key === "Escape") layer.remove(); });
  trapModalFocus(layer, dialog, close);
}
...
async function showAppSettings(): Promise<void> {
  completeSettings = await loadAppSettings();
  const managers = await listFileManagers().catch(() => []);
  openAppSettingsDialog(completeSettings, remotes, async next => {
...
function remoteCard(remote: Remote): HTMLElement {
  ...
  card.title = `Click to configure ${remote.name} (${remote.providerLabel}). Drag to move this remote.`;
...
function selectAdjacentRemote(delta: number): void {
  suppressRemoteHover();
  const query = remoteFilter.trim().toLocaleLowerCase();
  const visible = query ? [] : visibleRemotes();
...
function renderRemotePane(): HTMLElement {
  ...
  search.addEventListener("keydown", event => {
    if (event.key === "ArrowDown") { moveGlobalSearchSelection(1); event.preventDefault(); }
    if (event.key === "ArrowUp") { moveGlobalSearchSelection(-1); event.preventDefault(); }
    if (event.key === "Enter") {
      const result = globalSearchResults[Math.min(globalSearchSelected, 79)];
...
async function previewGlobalSearchResult(result: SearchEntry): Promise<void> {
  ...
  selectedRemote = result.remoteId;
  currentPath = result.isDir ? result.path : result.parentPath;
  if (!result.isDir) {
    localStorage.setItem(`mountlet-selection:${result.remoteId}:${result.parentPath}`, result.path);
    persistBrowserFolder(result.remoteId, result.parentPath, result.path, 0);
  } else persistBrowserPath(result.remoteId, currentPath);
  snapshot = folderSnapshots.get(`${selectedRemote}:${currentPath}`) ?? null;
  browserLoading = !snapshot;
  browserError = "";
  renderRemoteList();
  if (preferences.mode === "single") renderBrowserOnly();
...
function renderBrowserPane(): HTMLElement {
  ...
  search.addEventListener("keydown", event => {
    if (event.key === "ArrowDown") { moveRemoteSearchSelection(1); event.preventDefault(); }
    if (event.key === "ArrowUp") { moveRemoteSearchSelection(-1); event.preventDefault(); }
    if (event.key === "Enter") { const result = remoteSearchResults[Math.min(remoteSearchSelected, 49)]; if (result) void previewRemoteSearchResult(result, true); event.preventDefault(); }
    if (event.key === "Escape") {
      fileFilter = "";
      search.value = "";
  ...
  queueMicrotask(() => {
    ...
    viewport.addEventListener("keydown", event => {
      ...
      if (event.key === "Home") { fileList.select(0); event.preventDefault(); }
      if (event.key === "End") { fileList.select(Number.MAX_SAFE_INTEGER); event.preventDefault(); }
      if (event.key === "PageDown") { fileList.select(fileList.currentIndex() + fileList.pageSize()); event.preventDefault(); }
      if (event.key === "PageUp") { fileList.select(fileList.currentIndex() - fileList.pageSize()); event.preventDefault(); }
      if (event.key === "Backspace" || matchesShortcut(event, "browser_parent")) { void navigate(parentPath(currentPath)); event.preventDefault(); }
      if ((event.altKey && event.key === "Home") || matchesShortcut(event, "browser_root")) { void navigate(""); event.preventDefault(); }
      if (event.key === "F5" || matchesShortcut(event, "browser_refresh")) { void refreshFolder(); event.preventDefault(); }
      if (event.key === "F2") { if (requireIntegratedEdits()) fileList.renameCurrent(); event.preventDefault(); }
      if ((event.ctrlKey && event.key.toLocaleLowerCase() === "c") || matchesShortcut(event, "browser_copy")) { if (requireIntegratedEdits()) rememberFileClipboard(false); event.preventDefault(); }
      if (event.ctrlKey && event.key.toLocaleLowerCase() === "a") { fileList.selectAll(); event.preventDefault(); }
      if ((event.ctrlKey && event.key.toLocaleLowerCase() === "x") || matchesShortcut(event, "browser_cut")) { if (requireIntegratedEdits()) rememberFileClipboard(true); event.preventDefault(); }
      if ((event.ctrlKey && event.key.toLocaleLowerCase() === "v") || matchesShortcut(event, "browser_paste")) { if (requireIntegratedEdits()) void pasteFileClipboard(); event.preventDefault(); }
      if (event.key === "Delete" || matchesShortcut(event, "browser_delete")) { if (requireIntegratedEdits()) void deleteCurrentEntry(); event.preventDefault(); }
      if (matchesShortcut(event, "browser_new_folder")) { if (requireIntegratedEdits()) void makeFolder(); event.preventDefault(); }
      if ((event.ctrlKey && event.key === "Enter") || matchesShortcut(event, "browser_open_folder")) { void openMountedFolder(selectedRemote, currentPath); event.preventDefault(); }
      else if (event.key === "Enter" || matchesShortcut(event, "browser_open")) { const current = fileList.current(); if (current) void openEntry(current); event.preventDefault(); }
      if (event.key === "Escape") { focusRemoteList(); event.preventDefault(); }
      event.stopPropagation();
    });
  });
  return pane;
...
async function openSearchResult(result: SearchEntry): Promise<void> {
  ...
  localStorage.setItem(`mountlet-path:${result.remoteId}`, currentPath);
  remoteFilter = "";
  globalSearchResults = [];
  snapshot = folderSnapshots.get(`${selectedRemote}:${currentPath}`) ?? null;
  render();
...
async function navigate(path: string): Promise<void> {
  ...
  persistBrowserPath(selectedRemote, currentPath);
  snapshot = folderSnapshots.get(`${selectedRemote}:${currentPath}`) ?? null;
  browserError = "";
  browserLoading = !snapshot;
  renderBrowserOnly();
...
async function selectRemote(remoteId: string): Promise<void> {
  ...
  persistBrowserPath(remoteId, selectedPath);
  if (preferences.mode === "multiple") await emitBrowserState(selectedRemote, currentPath);
  else renderBrowserOnly();
  scheduleNativeLayout(0);
}
...
document.addEventListener("keydown", event => {
  ...
  if (!editing && ((!event.ctrlKey && !event.altKey && !event.metaKey && event.key.toLocaleLowerCase() === "f") || matchesShortcut(event, "common_search"))) {
    document.querySelector<HTMLInputElement>(isBrowserWindow || inBrowser ? ".browser-pane .search-input" : ".remote-pane .search-input")?.focus();
    event.preventDefault(); return;
  }
  if ((isBrowserWindow || inBrowser) && !editing && (event.key === "ArrowLeft" || event.key === "ArrowRight")) {
    if (directionPointsToMain(event.key)) focusRemoteList();
    event.preventDefault(); return;
  }
  if (!isBrowserWindow && !editing && !inBrowser && (event.key === "ArrowLeft" || event.key === "ArrowRight")) {
    if (directionPointsToBrowser(event.key)) focusBrowserList();
    event.preventDefault(); return;
  }
  if (!isBrowserWindow && !editing && !inBrowser && (event.key === "Enter" || event.key === " " || matchesShortcut(event, "remote_enter_browser"))) {
  ...
  if (event.key === "+" || event.key === "=") { setZoom(1); event.preventDefault(); }
  if (event.key === "-") { setZoom(-1); event.preventDefault(); }
  if (event.key === "0") { resetZoom(); event.preventDefault(); }
  if (event.key.toLocaleLowerCase() === "t") { cycleTheme(); event.preventDefault(); }
});
...
window.addEventListener("focus", () => {
  if (document.querySelector(".modal-layer")) return;
  if (isBrowserWindow) queueMicrotask(() => fileList?.focus());
  else queueMicrotask(() => focusRemoteList());
});
...
async function start(): Promise<void> {
  ...
  await listenFolderUpdated(async (remoteId, path) => {
    ...
    if (remoteId === selectedRemote && path === currentPath) {
      snapshot = refreshed; browserLoading = false; renderBrowserOnly(); scheduleNativeLayout(0);
    }
  });
  if (!isBrowserWindow) await listenRemoteUsageDirty(queueUsageRefresh);
  await listenNativeFileDrop(paths => {
    if (!paths.length || !selectedRemote || !preferences.integratedFileEdits) return;
    void (async () => {
      const remote = remotes.find(candidate => candidate.id === selectedRemote);
      if (!remote || !await confirmOwned("Upload local items?", `Upload ${paths.length} local item${paths.length === 1 ? "" : "s"} to ${remote.name}${currentPath ? `/${currentPath}` : ""}?`, "Upload")) return;
      try {
        await uploadLocalPaths(selectedRemote, currentPath, paths);
        await refreshFolder();
  ...
  if (!isBrowserWindow && (completeSettings?.noticeCheckInterval ?? 14400) > 0) {
