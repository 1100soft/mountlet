...
export async function createRemoteFile(remoteId: string, path: string, contents = ""): Promise<void> {
  if (inTauri) await invoke("create_file", { request: { remoteId, path, contents } });
}
...
export async function listenNativeFileDrop(handler: (drop: NativeFileDrop) => void): Promise<() => void> {
  ...
  return getCurrentWebviewWindow().onDragDropEvent(event => {
    ...
    handler({
      ...
      clientX: event.payload.position.x / scale,
      clientY: event.payload.position.y / scale,
    });
  });
}
...
export async function licenseDefaultDeviceLabel(): Promise<string> {
  return inTauri ? invoke<string>("license_default_device_label") : "Browser preview (Desktop)";
}
...
export async function bugReportPreview(kind: "bug" | "crash", message: string, contact: string, includeLogs: boolean, crash = ""): Promise<string> {
  return invoke<string>("bug_report_preview", { kind, message, contact, includeLogs, crash });
}
...
export async function markNotificationsSeen(keys: string[]): Promise<void> {
  if (inTauri && keys.length) await invoke("mark_notifications_seen", { keys });
}
...
export async function emitBrowserState(remoteId: string, path: string, focus = false): Promise<void> {
  if (!inTauri) return;
  await invoke("set_browser_window", { enabled: true });
  await invoke("set_browser_state", { remoteId, path });
  const { emitTo } = await import("@tauri-apps/api/event");
  await emitTo("browser", "browser-state", { remoteId, path, focus });
}
...
export async function listenBrowserState(handler: (value: { remoteId: string; path: string; focus?: boolean }) => void): Promise<() => void> {
  if (!inTauri) return () => undefined;
  const { listen } = await import("@tauri-apps/api/event");
  return listen<{ remoteId: string; path: string; focus?: boolean }>("browser-state", event => handler(event.payload));
}
...
export async function listenRestoreKeyboardFocus(handler: (firstShow: boolean) => void): Promise<() => void> {
  if (!inTauri) return () => undefined;
  const { listen } = await import("@tauri-apps/api/event");
  return listen<boolean>("restore-keyboard-focus", event => handler(event.payload));
}
